...
namespace Bonfire
{
    ...
    struct PointLight : LightSource
    {
        ...
        PointLight(glm::vec3 color = glm::vec3(255.0f), glm::vec3 position = glm::vec3(0.0f), glm::vec3 scale = glm::vec3(1.0f), float intensity = 1.0f)
            : color(color), position(position), scale(scale), intensity(intensity)
        {
            name = "Point Light";
        }
    };
